...
function GetGlossaryList() {
    let tablelGlossaryList = document.querySelector("table");
    let table = "<table><tr><th>Term</th><th>Class</th><th>Definition</th><th>Tag</th></tr>";

    glossary.forEach((item) => {
        table += `<tr><td>${item.term}</td><td>${item.class}</td><td>${item.definition}</td><td>${item.tags}</td></tr>`;
    });

    table += "</table>";
    tablelGlossaryList.innerHTML = table;

    let selectTag = document.querySelector("select");
    let select = '<select name="classes" id="classes" onchange="onChange()"><option value="">Choose a Class</option>';

    removeDupClass(glossary).forEach((item) => {
    ...
    removeDupTag(glossary).forEach((item) => {
        div += `<input type="checkbox" id="tags" name="${item}" value="${item}" onchange="onChange()">
...
function onChange() {
    var dropDownList, dropDownListValue, input, inputValue, table;
    ...
    checkBox.forEach((item) => {
        if (item.checked) {
            arrCheckedValue.push(item.value.toUpperCase());
        }
    });

    dropDownList = document.getElementById("idClass");
    dropDownListValue = dropDownList.value.toUpperCase();
    ...
    table += inputClassFilter(inputValue, dropDownListValue, arrCheckedValue, glossary);
    table += "</table>";
...
function removeDupTag(glossary) {
    let result = [];
    glossary.forEach((item) => {
        let arrTag = item.tags.toString().split(",");
        arrTag.forEach((item) => {
...
function inputClassFilter(inputValue, dropDownListValue, arrCheckedValue, glossary) {
    let table = "";

    glossary
        .filter((item) => {
            return (
                item.term.toUpperCase().indexOf(inputValue) > -1 ||
                item.class.toString().toUpperCase().indexOf(inputValue) > -1 ||
                item.definition.toUpperCase().indexOf(inputValue) > -1 ||
                item.tags.toString().toUpperCase().indexOf(inputValue) > -1
            );
        })
        .filter((item) => {
            return item.class.toString().toUpperCase().indexOf(dropDownListValue) > -1;
        })
        .filter((item) => {
            return item.tags.toString().toUpperCase().includes(arrCheckedValue);
        })
        .forEach((item) => {
            table += `<tr><td>${item.term}</td><td>${item.class}</td><td>${item.definition}</td><td>${item.tags}</td>`;
        });
    return table;
}
